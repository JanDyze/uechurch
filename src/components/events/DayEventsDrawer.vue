...
<template>
  <Transition name="drawer">
    <div
      v-if="show"
      class="day-events-drawer border-l-4 border-[#01779b] bg-gray-50 dark:bg-gray-900 max-w-xl w-[28rem] h-full flex flex-col flex-shrink-0 shadow-2xl shadow-[#01779b]/20"
    >
      <div class="flex-shrink-0 p-6 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ formattedSelectedDay }}</h2>
      ...
      <div class="flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700">
        <button
          @click="$emit('addEvent')"
          class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#01779b] text-white rounded-lg hover:bg-[#015a77] transition-colors"
        >
          <Plus class="h-5 w-5" />
          <span>Add Event</span>
        </button>
