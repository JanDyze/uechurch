<script setup>
// Search as a mode, opened from the floating button, the way People and
// Minutes do it. It reaches past the month on screen: "when am I on" is asked
// about the coming weeks, not about whichever month happens to be open.
...
// The placeholder has to teach what the box reaches: nobody tries typing a
// role or a song into it unless something says they can. Slashes between the
// examples, because the comma is the operator.
const placeholder = 'Name / role / song — comma for several'

const showCount = computed(() => Boolean(props.searchQuery.trim()))
const countLabel = computed(
  () => `${props.resultCount} ${props.resultCount === 1 ? 'service' : 'services'}`
)
...
<template>
  <div
    v-if="open"
    class="sticky top-0 z-40 mb-3 shrink-0 rounded-xl border border-gray-200/80 bg-white/95 px-2 py-2 shadow-sm backdrop-blur dark:border-gray-700 dark:bg-gray-900/95 sm:px-3"
  >
    <!-- Mobile -->
