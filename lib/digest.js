...
import { eventIconPngPath } from "./eventIcons.js";
import {
  collectOccurrences,
  monthBounds,
  addDays,
...
// A mark per row, so the list is scannable rather than a column of
// same-weight headings. A pre-rendered PNG rather than the Phosphor SVG the
// app draws: Gmail strips <svg> outright. The images are generated by
// `npm run build:icons` and served from our own origin, and every one of them
// resolves to a file that exists (see pngNameFor in lib/eventIcons.js).
//
// Clients that block remote images will drop it, which is why the row still
// carries the title and the coloured type badge as text.
const eventIcon = (event, appUrl) =>
  `<img src="${escapeHtml(appUrl)}${eventIconPngPath(event)}" width="18" height="18" alt=""
        style="display:inline-block;width:18px;height:18px;vertical-align:-3px;" />`;
 ...
 */
const eventRow = (event, { showDate = true, appUrl = "" } = {}) => {
      ...
      <tr>
        ...
        <td valign="top" style="padding:14px 14px 14px 10px;border-left:1px solid ${LINE};">
          <div style="font:700 15px/1.35 Helvetica,Arial,sans-serif;color:${INK};">
            ${eventIcon(event, appUrl)}&nbsp;${escapeHtml(event.title)}
          </div>
          ${
            event.location
...
export function buildEventDigest({ kind, today, church, occurrences, appUrl }) {
  ...
  if (kind === "today") {
    ...
    body =
      ...
      ) + eventTable(occurrences, { showDate: false, appUrl });
    ...
    body = dates
      .map(
        (date) =>
          ...
          eventTable(groupedByDate[date], { showDate: false, appUrl })
      )
      .join("");
    ...
    body = paragraph("Here is the week ahead.") + eventTable(occurrences, { showDate: true, appUrl });
...
export function buildActivityReport({ church, today, range, data, appUrl }) {
  ...
  if (upcoming.length) {
    ...
    sections.push(eventTable(upcoming, { showDate: true, appUrl }));
