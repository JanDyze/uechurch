...
/** Whether `schedule` generates an occurrence on the `YYYY-MM-DD` `dateString`. */
export const scheduleFallsOn = (schedule, dateString) => {
  const date = parseDateString(dateString);
  if (!date) return false;
  if ((typeof schedule?.weekday === "number" ? schedule.weekday : 0) !== date.getDay()) {
    return false;
  }
  return matchesOccurrence(date, Array.isArray(schedule?.occurrences) ? schedule.occurrences : []);
};

/**
 * Whether one occasion marks `date`, by either of the two rules it can carry.
 *
 * `occurrences` says which weeks of the month — communion on the first Sunday.
 * `dates` names the days outright, because the rest of the church calendar is
 * not cyclical: Christmas, an anniversary, the Sunday Teacher's Day is being
 * observed on. Those cannot be written as "the fourth Sunday" and hold true
 * next year, so they are stored as the dates they actually fall on.
 *
 * Deliberately not `matchesOccurrence` alone: an empty `occurrences` means
 * "every week" there, which would make a dated occasion mark every service.
 */
const occasionMarks = (occasion, date) => {
  const dates = Array.isArray(occasion?.dates) ? occasion.dates : [];
  const weeks = Array.isArray(occasion?.occurrences) ? occasion.occurrences : [];
  if (dates.includes(toDateString(date))) return true;
  if (weeks.length) return matchesOccurrence(date, weeks);
  // Neither rule given: marks every occurrence, the way an empty `occurrences`
  // reads on the schedule itself. A date list that simply missed does not.
  return dates.length === 0;
};

/**
 * What a schedule is additionally *doing* on `date` — communion on the first
 * Sunday, the birthday bash on the last, the anniversary on the day itself.
 *
 ...
 * whoever recorded it would have to pick which of the two to fill in. The same
 * goes for the occasions that only mark the day — Grandparents Day, Pastor's
 * Appreciation — where there is nothing to run, only something to say. So an
...
export const occasionsOn = (schedule, date) =>
  (Array.isArray(schedule?.occasions) ? schedule.occasions : [])
    .filter((occasion) => occasion?.label && occasionMarks(occasion, date))
    .map((occasion) => occasion.label);

/** "Sunday Service · Communion" — one gathering, told what it is that week. */
/** Between a gathering's name and its occasions: "Sunday Service · Communion". */
export const OCCASION_SEPARATOR = " · ";

export const occurrenceTitle = (title, occasions = []) =>
  [title, ...occasions].filter(Boolean).join(OCCASION_SEPARATOR);
